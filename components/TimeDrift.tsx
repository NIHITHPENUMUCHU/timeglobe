...
export default function TimeDrift() {
  ...
  return (
    <motion.div 
      className={`text-center p-8 min-h-screen flex flex-col items-center justify-center ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-900'}`}
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    ...
    >
      <div className="absolute top-4 right-4 flex space-x-4">
        ...
        <button onClick={() => setShowChallenge(!showChallenge)} className="p-2 rounded-full hover:bg-gray-700">
          <Clock size={24} />
        </button>
      </div>
      ...
      <TimezoneSlider setSelectedTimezone={setSelectedTimezone} isDarkMode={isDarkMode} />
      {showChallenge && (
        <EasterEggChallenge 
